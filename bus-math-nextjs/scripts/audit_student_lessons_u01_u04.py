...
OUT_ROOT = ROOT / "bus-math-nextjs" / "out" / "student"
SCREENSHOT_ROOT = ROOT / "screenshots" / "student-lessons-u01-u04-ui-audit-2026-04-07"
...
def enumerate_routes() -> list[tuple[str, str]]:
    ...
    for unit in UNITS:
        for html_path in sorted((OUT_ROOT / unit).glob("lesson*/**/index.html")):
            rel = html_path.relative_to(OUT_ROOT).as_posix()
            route = "/student/" + rel.removesuffix("index.html")
            routes.append((unit, route))
    return routes


def slug_for_route(route: str) -> str:
    parts = [part for part in route.strip("/").split("/") if part and part != "student"]
    if len(parts) < 2:
...
def ensure_dirs() -> None:
    for viewport in VIEWPORTS:
        for unit in UNITS:
            (SCREENSHOT_ROOT / viewport.name / unit).mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for viewport in VIEWPORTS:
            ...
            results: list[dict] = []
            try:
                for unit, route in routes:
                    page = context.new_page()
                    try:
                        screenshot_path = SCREENSHOT_ROOT / viewport.name / unit / f"{slug_for_route(route)}.png"
