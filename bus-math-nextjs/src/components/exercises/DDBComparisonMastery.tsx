...
function computeDDBSchedule(cost: number, salvage: number, life: number): { year: number; expense: number; accumulated: number; bookValue: number }[] {
  const slRate = 1 / life
...
function computeSLSchedule(cost: number, salvage: number, life: number): { year: number; expense: number; accumulated: number; bookValue: number }[] {
  ...
  for (let year = 1; year <= life; year++) {
    schedule.push({
      ...
      accumulated: annualExpense * year,
      bookValue: cost - annualExpense * year,
    })
  }
  return schedule
}
...
function generateProblem(): DDBProblem {
  ...
  return {
    ...
    correctDDBExpense: ddbRow.expense,
    correctSLExpense: slRow.expense,
    correctBookValue: ddbRow.bookValue,
    correctAccumulated: ddbRow.accumulated,
    method,
...
export default function DDBComparisonMastery() {
  ...
  }, [userAnswer, problem.distractors])
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 border border-amber-200 p-3 rounded">
              <p className="text-sm text-amber-800 font-medium">
                {problem.method === 'compare'
                  ? `Compare Year ${problem.yearToCalculate} under both methods.`
                  : `Calculate Year ${problem.yearToCalculate} using DDB.`}
              </p>
            </div>
            ...
            <div className="flex gap-2 justify-center pt-2">
              {!submitted ? (
                <>
                  <Button onClick={handleSubmit} disabled={!userAnswer} className="bg-purple-600 hover:bg-purple-700">
                    Check Answer
                  </Button>
                  <Button variant="outline" onClick={handleShowExample} className="border-purple-300 text-purple-700">
              ...
              <div className="flex-1">
                ...
                <div className="mt-2 bg-white p-3 rounded border border-green-200">
                  <p className="text-sm text-green-700">
                    <strong>DDB Year {problem.yearToCalculate}:</strong> ${problem.correctDDBExpense.toLocaleString()} expense, ${problem.correctAccumulated.toLocaleString()} accumulated, ${problem.correctBookValue.toLocaleString()} book value
                  </p>
                  <p className="text-sm text-green-700">
                    <strong>SL Year {problem.yearToCalculate}:</strong> ${problem.correctSLExpense.toLocaleString()} expense
                  </p>
                </div>
              </div>
            </div>
                ...
                <tbody>
                  {ddbSchedule.map((row, i) => (
                    <tr key={row.year} className={row.year === problem.yearToCalculate ? 'bg-purple-100' : ''}>
                      ...
                      <td className="border border-gray-300 p-2 text-right">${slSchedule[i].expense.toLocaleString()}</td>
                      <td className="border border-gray-300 p-2 text-right">${slSchedule[i].bookValue.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-purple-50 p-3 rounded border border-purple-200">
            ...
            <ul className="text-sm text-purple-700 space-y-1 list-disc list-inside">
              ...
              <li>Apply rate to beginning book value (not depreciable base)</li>
              <li>Do not subtract salvage value in the calculation</li>
          ...
          <div className="bg-orange-50 p-3 rounded border border-orange-200">
            ...
            <ul className="text-sm text-orange-700 space-y-1 list-disc list-inside">
              <li>Subtracting salvage value before applying DDB rate</li>
              <li>Using straight-line rate instead of double rate</li>
              <li>Letting book value fall below salvage value</li>
              <li>Applying the rate to cost every year instead of book value</li>
            </ul>
          </div>
        </div>
      </CardContent>
