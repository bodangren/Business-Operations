...
const capitalItems = [
  ...
  { desc: 'commercial refrigerator', costRange: [3000, 8000], lifeRange: [8, 12], salvageRange: [200, 800], reason: 'Commercial refrigerators are long-term equipment that should be capitalized.' },
  { desc: 'factory conveyor belt', costRange: [15000, 40000], lifeRange: [10, 20], salvageRange: [1000, 5000], reason: 'Factory equipment is a long-term asset that should be capitalized.' },
]
...
const expenseItems = [
  ...
  { desc: 'cleaning supplies', costRange: [40, 150], reason: 'Cleaning supplies are consumed quickly and should be expensed.' },
  { desc: 'advertising campaign', costRange: [500, 3000], reason: 'Advertising costs are period expenses, not long-term assets.' },
]
...
function generateScenario(): PurchaseScenario {
  ...
  if (isCapital) {
    ...
    return {
      ...
      isCapital: true,
      reason: item.reason,
      distractorReason: `Some might think this should be expensed because it costs money now. But ${item.desc.toLowerCase()} provides value for ${item.lifeRange[0]}+ years, so it must be capitalized.`,
    }
  } else {
    ...
    return {
      ...
      isCapital: false,
      reason: item.reason,
      distractorReason: `Some might think this should be capitalized because it costs money. But ${item.desc.toLowerCase()} is used up within the current period, so it must be expensed immediately.`,
    }
  }
}

export default function CapitalizationExpenseMastery() {
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (isCorrect) {
      ...
      setStreak(streak + 1)
      setMasteryProgress(Math.min(100, masteryProgress + 20))
    } else {
      setConsecutiveCorrect(0)
    }
  }, [userAnswer, scenario.isCapital, consecutiveCorrect, streak, masteryProgress])
        ...
        <div className="bg-white p-6 rounded-lg border border-amber-200">
          <div className="space-y-4">
            ...
            {scenario.isCapital && (
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="bg-gray-100 rounded-lg p-3">
                  ...
                  <p className="text-lg font-bold text-gray-800">${scenario.cost.toLocaleString()}</p>
                </div>
                <div className="bg-amber-100 rounded-lg p-3">
                  <p className="text-xs text-amber-600 uppercase">Useful Life</p>
                  <p className="text-lg font-bold text-amber-800">{scenario.usefulLife} yrs</p>
                </div>
                <div className="bg-green-100 rounded-lg p-3">
                  <p className="text-xs text-green-600 uppercase">Salvage Value</p>
                  <p className="text-lg font-bold text-green-800">${scenario.salvageValue.toLocaleString()}</p>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-3 justify-center pt-2">
              <Button
                onClick={() => { setUserAnswer('capitalize'); }}
                variant={userAnswer === 'capitalize' ? 'default' : 'outline'}
                className={`min-w-[140px] ${userAnswer === 'capitalize' ? 'bg-green-600 hover:bg-green-700' : 'border-green-300 text-green-700'}`}
                disabled={submitted}
              >
                Capitalize (Asset)
              </Button>
              <Button
                onClick={() => { setUserAnswer('expense'); }}
                variant={userAnswer === 'expense' ? 'default' : 'outline'}
                className={`min-w-[140px] ${userAnswer === 'expense' ? 'bg-red-600 hover:bg-red-700' : 'border-red-300 text-red-700'}`}
                disabled={submitted}
              >
                Expense Now
              </Button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className={`font-semibold ${correct ? 'text-green-800' : 'text-red-800'}`}>
                  {correct
                    ? `Correct! This should be ${scenario.isCapital ? 'capitalized' : 'expensed'}.`
                    : `Not quite. This should be ${scenario.isCapital ? 'capitalized' : 'expensed'}.`
                  }
                </p>
                <p className={`mt-2 text-sm ${correct ? 'text-green-700' : 'text-red-700'}`}>
                  {scenario.reason}
                </p>
                {!correct && (
          ...
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            <div className="text-sm text-blue-700 space-y-2">
              ...
              <p className="pt-2 font-medium text-blue-800">
                If YES to both → <strong>Capitalize</strong> (record as an asset, depreciate over useful life).
              </p>
              <p className="font-medium text-blue-800">
                If NO to either → <strong>Expense</strong> (record as a cost in the current period).
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-green-50 p-3 rounded border border-green-200">
            ...
            <ul className="text-sm text-green-700 space-y-1 list-disc list-inside">
              <li>Item lasts more than 1 year</li>
              <li>Cost is significant</li>
              <li>It is equipment, vehicles, buildings, or major improvements</li>
            </ul>
          </div>
          <div className="bg-red-50 p-3 rounded border border-red-200">
            <h4 className="font-semibold text-red-800 mb-1">Expense When...</h4>
            <ul className="text-sm text-red-700 space-y-1 list-disc list-inside">
              <li>Item is used up within the current period</li>
              <li>It is a routine operating cost</li>
              <li>It is rent, utilities, supplies, or minor repairs</li>
            </ul>
          </div>
        </div>
      </CardContent>
