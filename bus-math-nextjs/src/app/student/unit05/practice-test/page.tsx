...
import {
  drawRandomUnit05Phase5Questions,
  getUnit05Phase5Questions,
  toComprehensionCheckItems,
  type Unit05LessonId,
  type Unit05Phase5Question
} from "@/data/question-banks/unit05-phase5"
import { practiceTestLessonData, practiceTestPhases, unit05Data } from "./practice-test-data"
import { cn } from "@/lib/utils"
import { CheckCircle2, ListChecks, NotebookPen, RefreshCw, Sparkles, Target, Timer, Wand2 } from "lucide-react"

const PRACTICE_TEST_STORAGE_KEY = "unit05_practice_test_state_v1"
...
const lessonOptions: Array<{
  ...
  {
    id: "lesson01",
    title: "Lesson 01 – Payroll Cash Crunch",
    focus: "Cash flow timing and payroll obligations",
    color: "border-rose-300 bg-rose-50"
  },
  {
    id: "lesson02",
    title: "Lesson 02 – Gross to Net Calculations",
    focus: "Fundamental payroll mathematics and withholdings",
    color: "border-sky-300 bg-sky-50"
  },
  {
    id: "lesson03",
    title: "Lesson 03 – Prototype Calculator",
    focus: "Building single-employee payroll with IF logic",
    color: "border-emerald-300 bg-emerald-50"
  },
  {
    id: "lesson04",
    title: "Lesson 04 – Data Validation & Error Detection",
    focus: "Bulletproofing calculators with validation rules",
    color: "border-amber-300 bg-amber-50"
  },
  {
    id: "lesson05",
    title: "Lesson 05 – Payroll Tax & Overtime Engine",
    focus: "XLOOKUP, SUMPRODUCT, and payroll automation",
    color: "border-purple-300 bg-purple-50"
  },
  {
    id: "lesson06",
    title: "Lesson 06 – Payroll Decision Dashboard",
    focus: "Scenario switching, KPIs, and investor-ready visuals",
    color: "border-cyan-300 bg-cyan-50"
  },
  {
    id: "lesson07",
    title: "Lesson 07 – QA, Reconciliation, and Presentation",
    focus: "Validation, reconciliation, and executive summaries",
...
type StoredResult = {
  ...
  percentage: number
  completedAt: string
  lessonBreakdown: Record<Unit05LessonId, number>
}

type StoredState = {
  selectedLessonIds: Unit05LessonId[]
...
export default function PracticeTestPage() {
  ...
  const [selectedLessonIds, setSelectedLessonIds] = useState<Unit05LessonId[]>(defaultLessonSelection)
  const [questionCount, setQuestionCount] = useState<number>(12)
  const [activeQuestions, setActiveQuestions] = useState<Unit05Phase5Question[]>([])
  ...
  const availableQuestions = useMemo(() => {
    ...
    return getUnit05Phase5Questions({ lessonIds: selectedLessonIds })
  }, [selectedLessonIds])
  ...
  const handleToggleLesson = (lessonId: Unit05LessonId) => {
  ...
  const startPracticeTest = () => {
    ...
    const questions = drawRandomUnit05Phase5Questions(questionCount, {
  ...
  const lessonBreakdown = useMemo(() => {
    return activeQuestions.reduce<Record<Unit05LessonId, number>>((acc, question) => {
      acc[question.lessonId] = (acc[question.lessonId] ?? 0) + 1
      return acc
    }, {} as Record<Unit05LessonId, number>)
  }, [activeQuestions])

  const currentPhase = phasesForNavigation.find(phase => phase.sequence === 5)!

  const navigationOverrides = {
    lessonHref: "/student/unit05/practice-test",
  ...
  const footerNavigationOverrides = {
    ...
    phaseHrefBuilder: (phase: LessonPhase) => `/student/unit05/practice-test#phase-${phase.sequence}`
  }
          ...
          <section id="phase-1" className="space-y-6">
            <div className="text-center space-y-4">
              <Badge className="bg-rose-100 text-rose-800 text-lg px-4 py-2">
                ✨ Phase 1: Hook – HR Professional Spotlight
              </Badge>
              <Card className="max-w-4xl mx-auto border-rose-200 bg-white">
                <CardHeader>
                  <CardTitle className="flex items-center justify-center gap-2 text-rose-900">
                    ...
                    Sarah's payroll demo rehearsal starts now
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-left">
                  <p className="text-lg leading-relaxed text-slate-800">
                    Sarah Chen is preparing to demo TechStart Solutions' payroll system to a panel of HR professionals
                    and small business owners. They expect her to answer rapid-fire questions about gross pay, tax
                    withholdings, overtime calculations, and cash flow timing. This practice test is your chance to
                    rehearse alongside Sarah so both of you are ready for the real Q&amp;A.
                  </p>
                  <div className="bg-rose-50 border border-rose-200 rounded-lg p-4 text-rose-900">
                    <h3 className="font-semibold mb-2">Why this matters</h3>
                    <p className="text-sm leading-relaxed">
                      HR professionals and business owners judge a payroll system on accuracy, reliability, and
                      compliance. When you can explain every calculation without hesitation, you prove the system is
                      trustworthy and audit-ready. Your mastery keeps Sarah's credibility high and the conversation
                      focused on the value she delivers, not on errors that could have been caught with stronger
                      preparation.
                    </p>
                  </div>
                </CardContent>
                ...
                <Card className="border-sky-200 bg-white">
                  ...
                  <CardContent className="space-y-4 text-left text-slate-800">
                    <p className="text-lg leading-relaxed">
                      ...
                      reinforce before the HR professional demo.
                    </p>
                ...
                <Card className="border-emerald-200 bg-white">
                  ...
                  <CardContent className="grid gap-4 text-left text-slate-800">
                    <ul className="list-disc list-inside space-y-2 text-sm">
                      <li>
                        Commit to a steady pace. HR professionals expect confident answers in under a minute, so practice
                        finishing each item within 60 seconds.
                      </li>
                      <li>
                        Read the prompt, anchor it to Sarah's payroll system, then scan for the answer that protects
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-emerald-900">
                      ...
                      <p className="text-sm leading-relaxed">
                        Imagine an HR professional asks, "How do you ensure overtime rates are calculated correctly for
                        all employees?" Spend 30 seconds describing the validation you would spotlight first. That
                        mindset will guide you through the toughest questions ahead.
                      </p>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                {activeQuestions.length > 0 ? (
                  <ComprehensionCheck
                    ...
                    description="Answer the questions, then review the explanations to strengthen your payroll-ready story."
                    ...
                    {lastResult ? (
                      <div className="grid gap-4">
                        ...
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 text-orange-900 space-y-2">
                          ...
                          <p className="text-sm leading-relaxed">
                            A score of 80% or higher means you can defend Sarah's payroll system with confidence. If you
                            scored lower, focus on the lessons with the largest slice in the breakdown below before the
                            next professional demo rehearsal.
                          </p>
                        </div>
                        <div className="grid gap-3">
                          {lessonOptions.map(option => {
                ...
                <Card className="border-indigo-200 bg-white">
                  ...
                  <CardContent className="space-y-4 text-left text-slate-800">
                    <p className="text-lg leading-relaxed">
                      ...
                      Do the same here. The next professional demo depends on consistent reflection and rapid iteration.
                    </p>
                    <ReflectionJournal
                      unitTitle="Unit 5 Practice Test Reflection"
                      prompts={[
                        {
                          id: "unit05-practice-test-reflection-strength",
                          category: "courage",
                          prompt: "What payroll skill did you handle with confidence during this practice test, and how will you showcase it in the professional demo?",
                          placeholder: "Describe the question or concept you handled well and the evidence you will share with HR professionals..."
                        },
                        {
                          id: "unit05-practice-test-reflection-focus",
                          category: "persistence",
                          prompt: "Which payroll skill still needs polish before the next sprint check-in, and what action will you take to strengthen it?",
                    ...
                    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 text-indigo-900 space-y-2">
                      ...
                      <p className="text-sm leading-relaxed">
                        ...
                        payroll story tight and your Excel instincts sharp when the questions get tough.
                      </p>
                    </div>
                  </CardContent>
                </Card>
