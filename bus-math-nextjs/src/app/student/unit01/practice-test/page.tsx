...
import {
  drawRandomUnit01Phase5Questions,
  getUnit01Phase5Questions,
  toComprehensionCheckItems,
  type Unit01LessonId,
  type Unit01Phase5Question
} from "@/data/question-banks/unit01-phase5"
import { practiceTestLessonData, practiceTestPhases, unit01Data } from "./practice-test-data"
import { cn } from "@/lib/utils"
import { CheckCircle2, ListChecks, NotebookPen, RefreshCw, Sparkles, Target, Timer, Wand2 } from "lucide-react"

const PRACTICE_TEST_STORAGE_KEY = "unit01_practice_test_state_v1"
...
const lessonOptions: Array<{
  ...
  {
    id: "lesson01",
    title: "Lesson 01 – Smart Ledger Vision",
    focus: "Investor confidence and ledger storytelling",
    color: "border-rose-300 bg-rose-50"
  },
  {
    id: "lesson02",
    title: "Lesson 02 – Accounting Equation",
    focus: "Balancing assets, liabilities, and equity in every move",
    color: "border-sky-300 bg-sky-50"
  },
  {
    id: "lesson03",
    title: "Lesson 03 – Debit and Credit Rules",
    focus: "DEA LER mastery for perfect transaction entries",
    color: "border-emerald-300 bg-emerald-50"
  },
  {
    id: "lesson04",
    title: "Lesson 04 – Excel Tables and SUMIF",
    focus: "Automating structure with tables, named ranges, and SUMIF",
  ...
  {
    id: "lesson06",
    title: "Lesson 06 – Integration and Dashboards",
    focus: "Scenario switching, KPI dashboards, and investor demos",
    color: "border-cyan-300 bg-cyan-50"
  },
  {
    id: "lesson07",
    title: "Lesson 07 – Investor Readiness and Audit",
    focus: "Peer audits, XLOOKUP safety nets, and audit trails",
...
type StoredResult = {
  ...
  percentage: number
  completedAt: string
  lessonBreakdown: Record<Unit01LessonId, number>
}

type StoredState = {
  selectedLessonIds: Unit01LessonId[]
...
export default function PracticeTestPage() {
  ...
  const [selectedLessonIds, setSelectedLessonIds] = useState<Unit01LessonId[]>(defaultLessonSelection)
  const [questionCount, setQuestionCount] = useState<number>(12)
  const [activeQuestions, setActiveQuestions] = useState<Unit01Phase5Question[]>([])
  ...
  const availableQuestions = useMemo(() => {
    ...
    return getUnit01Phase5Questions({ lessonIds: selectedLessonIds })
  }, [selectedLessonIds])
  ...
  const handleToggleLesson = (lessonId: Unit01LessonId) => {
  ...
  const startPracticeTest = () => {
    ...
    const questions = drawRandomUnit01Phase5Questions(questionCount, {
  ...
  const lessonBreakdown = useMemo(() => {
    return activeQuestions.reduce<Record<Unit01LessonId, number>>((acc, question) => {
      acc[question.lessonId] = (acc[question.lessonId] ?? 0) + 1
      return acc
    }, {} as Record<Unit01LessonId, number>)
  }, [activeQuestions])

  const currentPhase = phasesForNavigation.find(phase => phase.sequence === 5)!

  const navigationOverrides = {
    lessonHref: "/student/unit01/practice-test",
  ...
  const footerNavigationOverrides = {
    ...
    phaseHrefBuilder: (phase: LessonPhase) => `/student/unit01/practice-test#phase-${phase.sequence}`
  }
              ...
              <Card className="max-w-4xl mx-auto border-rose-200 bg-white">
                <CardHeader>
                  <CardTitle className="flex items-center justify-center gap-2 text-rose-900">
                    ...
                    Sarah's investor rehearsal starts now
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-left">
                  <p className="text-lg leading-relaxed text-slate-800">
                    Sarah Chen is three days away from pitching TechStart Solutions to a panel of angel investors. She
                    already built the Smart Ledger, but investors expect her to answer rapid-fire questions about every
                    formula, chart, and cash balance. This practice test is your chance to rehearse alongside Sarah so
                    both of you are ready for the real Q&amp;A.
                  </p>
                  <div className="bg-rose-50 border border-rose-200 rounded-lg p-4 text-rose-900">
                    <h3 className="font-semibold mb-2">Why this matters</h3>
                    <p className="text-sm leading-relaxed">
                      Investors judge a startup on clarity, speed, and confidence. When you can explain the ledger’s
                      math without hesitation, you prove the business is disciplined and audit-ready. Your mastery keeps
                      Sarah's credibility high and the conversation focused on the value she delivers, not on errors that
                      could have been caught with stronger preparation.
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <Card className="border-emerald-200 bg-white">
                  ...
                  <CardContent className="grid gap-4 text-left text-slate-800">
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-emerald-900">
                      ...
                      <p className="text-sm leading-relaxed">
                        Imagine an investor asks, “How do you prove this dashboard updates correctly every time new sales
                        arrive?” Spend 30 seconds describing the control you would spotlight first. That mindset will
                        guide you through the toughest questions ahead.
                      </p>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                {activeQuestions.length > 0 ? (
                  <ComprehensionCheck
                    ...
                    description="Answer the questions, then review the explanations to strengthen your investor-ready story."
                    ...
                    {lastResult ? (
                      <div className="grid gap-4">
                        ...
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 text-orange-900 space-y-2">
                          ...
                          <p className="text-sm leading-relaxed">
                            A score of 80% or higher means you can defend Sarah’s ledger with confidence. If you scored
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                <Card className="border-indigo-200 bg-white">
                  ...
                  <CardContent className="space-y-4 text-left text-slate-800">
                    ...
                    <ReflectionJournal
                      unitTitle="Unit 1 Practice Test Reflection"
                      prompts={[
                        {
                          id: "unit01-practice-test-reflection-strength",
                          category: "courage",
                          prompt: "What ledger move did you handle with confidence during this practice test, and how will you showcase it in the investor demo?",
                          placeholder: "Describe the question or concept you handled well and the evidence you will share with investors..."
                        },
                        {
                          id: "unit01-practice-test-reflection-focus",
                          category: "persistence",
                          prompt: "Which ledger skill still needs polish before the next sprint check-in, and what action will you take to strengthen it?",
                    ...
                    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 text-indigo-900 space-y-2">
                      ...
                      <p className="text-sm leading-relaxed">
                        ...
                        investor story tight and your Excel instincts sharp when the questions get tough.
                      </p>
                    </div>
                  </CardContent>
                </Card>
