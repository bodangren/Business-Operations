...
import {
  drawRandomUnit03Phase5Questions,
  getUnit03Phase5Questions,
  toComprehensionCheckItems,
  type Unit03LessonId,
  type Unit03Phase5Question
} from "@/data/question-banks/unit03-phase5"
import { practiceTestLessonData, practiceTestPhases, unit03Data } from "./practice-test-data"
import { cn } from "@/lib/utils"
import { CheckCircle2, ListChecks, NotebookPen, RefreshCw, Sparkles, Target, Timer, Wand2 } from "lucide-react"

const PRACTICE_TEST_STORAGE_KEY = "unit03_practice_test_state_v1"
...
const lessonOptions: Array<{
  ...
  {
    id: "lesson01",
    title: "Lesson 01 – Three-Statement Storyboard Vision",
    focus: "Integrated financial statements tell one cohesive business story",
    color: "border-rose-300 bg-rose-50"
  },
  {
    id: "lesson02",
    title: "Lesson 02 – Income Statement Construction",
    focus: "Building dynamic Income Statements with INDEX/MATCH automation",
    color: "border-sky-300 bg-sky-50"
  },
  {
    id: "lesson03",
    title: "Lesson 03 – Dynamic Income Statement with INDEX/MATCH",
    focus: "Professional-grade formulas and investor-ready presentation",
    color: "border-emerald-300 bg-emerald-50"
  },
  {
    id: "lesson04",
    title: "Lesson 04 – Advanced INDEX/MATCH & Named Ranges",
    focus: "Named ranges, error handling, and maintainable models",
    color: "border-amber-300 bg-amber-50"
  },
  {
    id: "lesson05",
    title: "Lesson 05 – Three-Statement Automation",
    focus: "XLOOKUP, scenario switching, and validation frameworks",
    color: "border-purple-300 bg-purple-50"
  },
  {
    id: "lesson06",
    title: "Lesson 06 – Integration & Dashboard Development",
    focus: "KPI dashboards, visual storytelling, and investor demos",
    color: "border-cyan-300 bg-cyan-50"
  },
  {
    id: "lesson07",
    title: "Lesson 07 – Audit, Review, & Investor Presentation",
    focus: "Error handling, audit-readiness, and professional delivery",
...
type StoredResult = {
  ...
  percentage: number
  completedAt: string
  lessonBreakdown: Record<Unit03LessonId, number>
}

type StoredState = {
  selectedLessonIds: Unit03LessonId[]
...
export default function PracticeTestPage() {
  ...
  const [selectedLessonIds, setSelectedLessonIds] = useState<Unit03LessonId[]>(defaultLessonSelection)
  const [questionCount, setQuestionCount] = useState<number>(12)
  const [activeQuestions, setActiveQuestions] = useState<Unit03Phase5Question[]>([])
  ...
  const availableQuestions = useMemo(() => {
    ...
    return getUnit03Phase5Questions({ lessonIds: selectedLessonIds })
  }, [selectedLessonIds])
  ...
  const handleToggleLesson = (lessonId: Unit03LessonId) => {
  ...
  const startPracticeTest = () => {
    ...
    const questions = drawRandomUnit03Phase5Questions(questionCount, {
  ...
  const lessonBreakdown = useMemo(() => {
    return activeQuestions.reduce<Record<Unit03LessonId, number>>((acc, question) => {
      acc[question.lessonId] = (acc[question.lessonId] ?? 0) + 1
      return acc
    }, {} as Record<Unit03LessonId, number>)
  }, [activeQuestions])

  const currentPhase = phasesForNavigation.find(phase => phase.sequence === 5)!

  const navigationOverrides = {
    lessonHref: "/student/unit03/practice-test",
  ...
  const footerNavigationOverrides = {
    ...
    phaseHrefBuilder: (phase: LessonPhase) => `/student/unit03/practice-test#phase-${phase.sequence}`
  }
              ...
              <Card className="max-w-4xl mx-auto border-rose-200 bg-white">
                <CardHeader>
                  <CardTitle className="flex items-center justify-center gap-2 text-rose-900">
                    ...
                    Sarah's three-statement storyboard goes live
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-left">
                  <p className="text-lg leading-relaxed text-slate-800">
                    Sarah Chen is preparing for her biggest investor presentation yet. The panel wants to see how every
                    journal entry flows into the Income Statement, Balance Sheet, and Statement of Cash Flows. They'll
                    ask rapid-fire questions about formula logic, statement integration, and dashboard automation. This
                    practice test is your chance to rehearse alongside Sarah so both of you can tell one cohesive financial story.
                  </p>
                  <div className="bg-rose-50 border border-rose-200 rounded-lg p-4 text-rose-900">
                    <h3 className="font-semibold mb-2">Why this matters</h3>
                    <p className="text-sm leading-relaxed">
                      Investors judge a startup on how well the financial statements integrate and tell one coherent story.
                      When you can explain how profit flows to retained earnings, how depreciation affects both income and
                      cash flow, and how the accounting equation stays balanced, you prove the business model is sound and
                      the management team understands the numbers that drive decisions.
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <Card className="border-emerald-200 bg-white">
                  ...
                  <CardContent className="grid gap-4 text-left text-slate-800">
                    <ul className="list-disc list-inside space-y-2 text-sm">
                      <li>
                        Commit to a steady pace. Investors expect confident answers about statement integration in under
                        a minute, so practice finishing each item within 60 seconds.
                      </li>
                      <li>
                        Read the prompt, connect it to Sarah's three-statement storyboard, then scan for the answer that
                        proves integration and maintains accuracy.
                      </li>
                      <li>
                        Use the explanations after you submit. They show the exact reasoning Sarah will share in the
                        investor meeting about how the statements connect.
                      </li>
                      <li>
                    ...
                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 text-emerald-900">
                      ...
                      <p className="text-sm leading-relaxed">
                        Imagine an investor asks, "How do you prove that your Income Statement, Balance Sheet, and Cash
                        Flow Statement all tell the same business story?" Spend 30 seconds describing the key integration
                        points you would spotlight first. That mindset will guide you through the toughest questions ahead.
                      </p>
                    </div>
                  </CardContent>
                </Card>
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                {activeQuestions.length > 0 ? (
                  <ComprehensionCheck
                    ...
                    description="Answer the questions, then review the explanations to strengthen your three-statement storyboard."
                    ...
                    {lastResult ? (
                      <div className="grid gap-4">
                        ...
                        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 text-orange-900 space-y-2">
                          ...
                          <p className="text-sm leading-relaxed">
                            A score of 80% or higher means you can defend Sarah's three-statement storyboard with confidence.
                            If you scored lower, focus on the lessons with the largest slice in the breakdown below before
                            the next investor rehearsal.
                          </p>
                        </div>
                        <div className="grid gap-3">
                          {lessonOptions.map(option => {
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                <Card className="border-indigo-200 bg-white">
                  ...
                  <CardContent className="space-y-4 text-left text-slate-800">
                    ...
                    <ReflectionJournal
                      unitTitle="Unit 3 Practice Test Reflection"
                      prompts={[
                        {
                          id: "unit03-practice-test-reflection-strength",
                          category: "courage",
                          prompt: "What three-statement integration move did you handle with confidence during this practice test, and how will you showcase it in the investor demo?",
                          placeholder: "Describe the question or concept you handled well and the evidence you will share with investors..."
                        },
                        {
                          id: "unit03-practice-test-reflection-focus",
                          category: "persistence",
                          prompt: "Which statement integration skill still needs polish before the next sprint check-in, and what action will you take to strengthen it?",
                    ...
                    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 text-indigo-900 space-y-2">
                      ...
                      <p className="text-sm leading-relaxed">
                        ...
                        financial storyboard tight and your Excel integration instincts sharp when the questions get tough.
                      </p>
                    </div>
                  </CardContent>
                </Card>
