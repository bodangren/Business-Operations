...
- [x] Debug routes middleware gating doesn't work with `output: 'export'` — middleware doesn't run during static export, so all `/debug/*` pages are served as static HTML on GitHub Pages. **Accepted as known limitation** (L1 from review, 2026-04-08)
- [ ] Teacher guidance fields unpopulated across 80 daily lessons — `preparation`, `facilitationGuidance`, `checksForUnderstanding`, `watchFors`, and `nextSteps` are defined in `DailyLesson` type but not populated in any unit lesson plan (L2 from current directive, 2026-04-08; see `measure/tracks/defer_teacher_guidance_fields_20260408/future-work.md` for scope)
